...
void write_to_line(int line, int value) {
  write_to_matrix(0, 0, 0, 0, 0, 0, 0, 0);// Leeres Array für die Zeilen der Matrix
  write_to_shifter(255 - value);//Durch das Shiftregister werden die Signale durch die Matrix geschoben
  write_to_matrix(line == 0, line == 1,//Gibt die Zeilen der Matrix an
                  line == 2, line == 3, line == 4,
                  line == 5, line == 6, line == 7);
}

void write_to_matrix(int v1, int v2,//Ausgabe durch Leuchten der Matrix
...
void write_to_shifter(int value) {
  for (int i = 7; i >= 0; --i) {
    digitalWrite(shcp, LOW); //Shiftpin aus 
    int b = value & (1 << i);
    digitalWrite(ds, b); //Datatpin gibt die Led an die leuchten soll
    digitalWrite(shcp, HIGH);//Shiftpin an durch den Flankenwechsel wird der Wert ausgegeben
  }
  digitalWrite(stcp, HIGH); // Storepin aus
  digitalWrite(stcp, LOW); // Storepin an durch den Flankenwechsel wird der Wert eingelesen
}
